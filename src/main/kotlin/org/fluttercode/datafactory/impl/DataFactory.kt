...
open class DataFactory protected constructor(random: Random) {
    ...
    fun <T> getItem(items: List<T>, defaultItem: T): T {
        return getItem(items, 100, defaultItem)
    }
    ...
    fun <T> getItem(items: Array<T>, defaultItem: T): T {
        return getItem(items, 100, defaultItem)
    }
    ...
    val firstName: String
        get() = getItem(nameDataValues.firstNames, "")
     ...
     */
    val name: String
        get() = getItem(nameDataValues.firstNames, "").toString() +
                " " +
                getItem(nameDataValues.lastNames, "")

    /**
     * @return A random last name
     */
    val lastName: String
        get() = getItem(nameDataValues.lastNames, "")

    /**
     * @return A random street name
     */
    val streetName: String
        get() = getItem(addressDataValues.streetNames, "")

    /**
     * @return A random street suffix
     */
    val streetSuffix: String
        get() = getItem(addressDataValues.addressSuffixes, "")
     ...
     */
    val city: String
        get() = getItem(addressDataValues.cities, "")
    ...
    fun getAddressLine2(probability: Int): String {
        return getAddressLine2(probability, "")
    }
     ...
     */
    fun getDateBetween(minDate: Date, maxDate: Date): Date { // this can break if seconds is an int
    ...
    val randomWord: String
        get() = getItem(contentDataValues.words, "")
    ...
    fun getRandomWord(minLength: Int, maxLength: Int): String {
        ...
        val words = contentDataValues.words
    ...
    fun getSuffix(chance: Int): String {
        return getItem(nameDataValues.suffixes, chance, "")
    }
    ...
    fun getPrefix(chance: Int): String {
        return getItem(nameDataValues.prefixes, chance, "")
    }
    ...
    val businessName: String
        get() = city + " " + getItem(contentDataValues.businessTypes, "")// 2 words// name and initial
    ...
    val emailAddress: String
        get() {
            val test = random.nextInt(100)
            var email = ""
            email = if (test < 50) { // name and initial
                firstName[0].toString() + lastName
            } else { // 2 words
                getItem(contentDataValues.words, "") +
                        getItem(contentDataValues.words, "")
            }
            if (random.nextInt(100) > 80) {
                email += random.nextInt(100)
            }
            email = email + "@" + getItem(contentDataValues.emailHosts, "") + "." +
                    getItem(contentDataValues.tlds, "")
